...
export function ChangeEventDrawer() {
  ...
  return <ChangeEventPanel key={event.id} event={event} onClose={() => patch({ changeEvent: null })} />
}
...
function ChangeEventPanel({ event: e, onClose }: { event: ChangeEvent; onClose: () => void }) {
  ...
  const openCommitment = (commitmentId: string | null) => {
    ...
    if (c) patch({ changeEvent: null, commitment: c })
  }
    ...
    <Backdrop onClose={onClose}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '15px 18px', borderBottom: '1px solid var(--bd-2)', background: '#fff', flex: 'none' }}>
          <CodeBadge code={TOOLS.changeEvents.code} style={{ fontSize: 10, fontWeight: 700, padding: '3px 7px', borderRadius: 5 }} />
          <span style={{ fontFamily: mono, fontSize: 12.5, fontWeight: 650, color: 'var(--tx-secondary)' }}>{e.number}</span>
          <div style={{ flex: 1 }} />
          <button
            type="button"
