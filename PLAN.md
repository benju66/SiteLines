...
| 2 | **Persist UI state** — hydrate/write-back `sidebarCollapsed` · `drawerWidth` · `drawerFull` + **column widths** (refactor `BudgetView`'s `widthsRef` → `settings.columnWidths[tableId]`, save on drag-end). AppState stays runtime; settings is a thin bridge | ⏳ Planned |
