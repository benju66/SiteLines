...
| 4 | **Project-switcher fix** — pure `activeProjects(itemsByTool)` → `SCOPES = ['all', …active]` so empty projects (McKenna) vanish; guard `state.project` if its scope disappears. Keeps the `Project` union for v1 (a dynamic `sitelines_projects` registry is a later ⛔ upgrade). Independent of the settings store — can run standalone | ⏳ Planned |
