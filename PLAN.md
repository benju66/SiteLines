...
| 6 | Scope list formatting + your-own-notes — bullets, opt-in numbered items, and user-authored "notes" pinned into a commitment's scope (extends the Phase-5 editor; contract words stay locked, notes shown clearly marked). Sub-phased **6a** list styling (bullets + numbers, presentation-only) · **6b** notes (the one words-locked safety-model change) | 🚧 **6a shipped** (list styling) — `list?: 'bullet'\|'number'` on `ScopeBlockOverride` + `ScopeBlock`; pure tested [annotateOrdinals](src/lib/applyScopeOverride.ts) (counter per indent · nested runs restart · any non-number block breaks the run); `ScopeOutline` draws `•` / the computed ordinal on **para** blocks (headings unaffected); [scopeEdit](src/lib/scopeEdit.ts) `setList` op + a plain→bullet→number cycle control (disabled on headings); `coerceBlocks` validates `list`. **Presentation-only** — the marker is drawn at render, never in `text`, so `partitionsSource` is untouched. 169 tests green; verified seed (`:5174`) + live (`:5175`, authenticated write to the real table, words verbatim, styles persist, advisors clean). **6b planned** ([Commitments-Plan §Phase 6](Notes/plans/Commitments-Plan.md)) — `source:'user'` notes: the one words-locked change (`partitionsSource` must exclude `source:'user'` blocks before asserting). No ⛔ SQL gate (`blocks` is `jsonb`; CHECK only on `field`) |
