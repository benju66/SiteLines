...
| 3 | Sync change (in-repo `sync/`) — per-commitment detail (show) pull → `procore_commitment_line_items_master` (cost code + amount + `budget_line_item_id`) + inclusions/exclusions onto commitment `raw` | 🚧 Implemented — [migration 0009](sync/migrations/0009_commitment_line_items.sql) applied + `procore_pipeline.py` updated; ⛔ owner runs re-sync + confirms Procore *show* scope (step 0 found the real sync is in-repo, not FP-Analytics) |
