...
### Parallel workstream: User Settings & UX
Shift from tool enrichment to the app's **preferences layer + UX polish** — its own plan:
[Notes/plans/Settings-And-UX-Plan.md](Notes/plans/Settings-And-UX-Plan.md). Chosen (owner,
2026-07-20) after the tool-enrichment run. One unifying theme: a **user-settings store** the
rest plugs into. **Persist to localStorage for v1** (per-user Supabase later); **foundation
first**. Includes the **project-switcher fix**: the scope list is HARDCODED (`SCOPES` in
`Sidebar.tsx` + `projectMeta` in `tokens.ts`), so **McKenna Crossing** (zero synced data)
shows as an empty scope — v1 **data-derives** the visible scopes (empty ones vanish; no new
Supabase view). No re-sync; no ⛔ gates (localStorage only).

| Phase | Surface | Status |
|-------|---------|--------|
| 1 | **Settings foundation** — `SettingsSource` seam + `localSettings()` (localStorage) + `SettingsProvider`/`useSettings()` + pure `defaultSettings`/`coerceSettings` (corruption-safe, +tests) + a **Settings overlay** (gear in Header → `position:fixed` panel) with Reset + one setting wired end-to-end (`sidebarCollapsed`). Models the scope-editor UserData seam | ✅ Shipped (2026-07-20) — typecheck + **305 tests** + build green; seed `:5174` + reload — gear opens the Settings overlay, "Collapse sidebar by default" persists to `sitelines.settings` and the collapsed rail survives a plain reload, Reset restores defaults, a hand-corrupted blob still boots to defaults with no console errors; overlay is a `position:fixed` sibling outside the card. No backend (localStorage only). Committed on `settings-phase-1` |
| 2 | **Persist UI state** — hydrate/write-back `sidebarCollapsed` · `drawerWidth` · `drawerFull` + **column widths** (refactor `BudgetView`'s `widthsRef` → `settings.columnWidths[tableId]`, save on drag-end). AppState stays runtime; settings is a thin bridge | ✅ Shipped (2026-07-20) — typecheck + **319 tests** + build green. `UserSettings` → **v2** (drawerWidth/drawerFull/columnWidths) with a v1→v2 migration that preserves a Phase-1 user's `sidebarCollapsed`; new pure `applyToTable` + coerce (both tested — junk/wrong-length columnWidths → defaults, out-of-range drawerWidth clamped). Thin `SettingsBridge` (pure `persistedFromState` + a subscriber) mirrors AppState's durable subset out; boot-hydrate seeds all three. Seed `:5174` + reload proved every path: sidebar collapse (its own button now durable), Budget column drag, drawer resize (452→592) + full-width toggle — all remembered; v1 blob migrates live; corrupt blob boots to defaults (0 console errors); Reset restores defaults; a fresh profile writes nothing until first change. No backend. Committed on `settings-phase-2` |
| 3 | **Pin / reorder tools** — `pinnedTools` setting + a pin control + a pure `orderedNav` surfacing pinned tools in the Sidebar (drag-reorder optional) | ✅ Shipped (2026-07-20) — typecheck + **328 tests** + build green. `UserSettings` → **v3** (`pinnedTools`) with a v2→v3 migration (lossless — preserves sidebar/drawer/column state); pure `orderedNav(groups, pinned)` lifts pinned tools into a "Pinned" section + filters stale keys (both tested). Hover-reveal pin toggle per tool (a sibling `<button>`, never nested); pinned rows keep their ball-in-court count. Seed `:5174` + reload proved: pin/unpin persists, pinned tools surface once (removed from their group) in pin order, nav + count badges still work, a stale/bogus pinned key yields no phantom item, a v2 blob migrates live. Pin-toggle only (v1 floor; drag-reorder deferred). No backend. Committed on `settings-phase-3` |
| 4 | **Project-switcher fix** — pure `activeProjects(itemsByTool)` → `SCOPES = ['all', …active]` so empty projects (McKenna) vanish; guard `state.project` if its scope disappears. Keeps the `Project` union for v1 (a dynamic `sitelines_projects` registry is a later ⛔ upgrade). Independent of the settings store — can run standalone | ✅ Shipped (2026-07-20) — typecheck + **333 tests** + build green. Pure `activeProjects(itemsByTool)` (canonical order, empty scopes excluded — unit-tested); the Sidebar's scope list is now `['all', …activeProjects]` + a `useEffect` guard resets `state.project` to `'all'` if its scope disappears. No settings store touched. Seed `:5174` — both projects show (seed has both), switching still filters (McKenna 13 + OP III 9 = 22). Live premise **DB-verified**: `sitelines_items` holds only `opiii` (3129 rows, 0 McKenna) → McKenna vanishes in live (full live browser render login-gated). No backend. Committed on `settings-phase-4` |
